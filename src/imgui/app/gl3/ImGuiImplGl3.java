//Rendering using openGL 3 is backed in the native library

//package imgui.app.gl3;
//
//import imguijb.*;
...
//            io.setBackendFlags(io.getBackendFlags() | ImGuiBackendFlags_.ImGuiBackendFlags_RendererHasVtxOffset);
//        }
//
//        // We can create multi-viewports on the Renderer side (optional)
//        io.setBackendFlags(io.getBackendFlags() | ImGuiBackendFlags_.ImGuiBackendFlags_RendererHasViewports);
//
//        // We can honor ImGuiPlatformIO::Textures[] requests during render.
//        io.setBackendFlags(io.getBackendFlags() | ImGuiBackendFlags_.ImGuiBackendFlags_RendererHasTextures);
//
//        if (glslVersion == null) {
//            if (IS_APPLE) {
...
//        io.setBackendFlags(io.getBackendFlags()%~(ImGuiBackendFlags_.ImGuiBackendFlags_RendererHasVtxOffset | ImGuiBackendFlags_.ImGuiBackendFlags_RendererHasViewports));
//        data = null;
//    }
//
...
//        var size = drawData.getDisplaySize();
//        float L = pos.getX();
//        float R = pos.getX() + size.getX();
//
//        float T = pos.getY();
//        float B = pos.getY() + size.getY();
//
...
//        glVertexAttribPointer(data.attribLocationVtxPos, 2, GL_FLOAT, false, ImGuiUtil.ImDrawVert__, 0);
//        glVertexAttribPointer(data.attribLocationVtxUV, 2, GL_FLOAT, false, ImGuiUtil.ImDrawVert__, 8);
//        glVertexAttribPointer(data.attribLocationVtxColor, 4, GL_UNSIGNED_BYTE, true, ImGuiUtil.ImDrawVert__, 16);
//    }
//
//    /**
...
//        var displaySize = drawData.getDisplaySize();
//        var frambscale = drawData.getFramebufferScale();
//        final int fbWidth = (int) (displaySize.getX() * frambscale.getX());
//        final int fbHeight = (int) (displaySize.getY() * frambscale.getY());
...
//        if(drawData.getTextures() != null) {
//            var txt = drawData.getTextures();
...
//        var displayPos = drawData.getDisplayPos();
//        final float clipOffX = displayPos.getX(); // (0,0) unless using multi-viewports
//        final float clipOffY = displayPos.getY(); // (0,0) unless using multi-viewports
//
//        final float clipScaleX = frambscale.getX(); // (1,1) unless using retina display which are often (2,2)
//        final float clipScaleY = frambscale.getY(); // (1,1) unless using retina display which are often (2,2)
//
//        final var lists = new ImGuiUtil.ImDrawListVector(drawData.getCmdLists());
...
//
//            var list = lists.get(n);
//            var vtx = new ImGuiUtil.ImGuiImVertVector(list.getVtxBuffer());
//            var indx = new ImGuiUtil.ImGuiShortVector(list.getIdxBuffer());
//            glBufferData(GL_ARRAY_BUFFER, vtx.data().asByteBuffer(), GL_STREAM_DRAW);
//            glBufferData(GL_ELEMENT_ARRAY_BUFFER, indx.data().asByteBuffer(), GL_STREAM_DRAW);
//
//            var cmdbuf = new ImGuiUtil.ImDrawCmdVector(list.getCmdBuffer());
//
//            for (int cmdIdx = 0; cmdIdx < cmdbuf.size(); cmdIdx++) {
//                // TODO:
//                // if userCallback
//                // else
//                var cmd = cmdbuf.get(cmdIdx);
//                var rect = cmd.getClipRect();
//                props.clipRect.setX(rect.getX());
//                props.clipRect.setY(rect.getY());
//                props.clipRect.setZ(rect.getZ());
//                props.clipRect.setW(rect.getW());
////                drawData.getCmdListCmdBufferClipRect(props.clipRect, n, cmdIdx);
//
//                final float clipMinX = (props.clipRect.getX() - clipOffX) * clipScaleX;
//                final float clipMinY = (props.clipRect.getY() - clipOffY) * clipScaleY;
//                final float clipMaxX = (props.clipRect.getZ() - clipOffX) * clipScaleX;
//                final float clipMaxY = (props.clipRect.getW() - clipOffY) * clipScaleY;
//
...
//
//                // Bind texture, Draw
//                final long textureId = /*drawData.getCmdListCmdBufferTextureId(n, cmdIdx)*/cmd.getTexRef().GetTexID().intValue();
//                final int elemCount = /*drawData.getCmdListCmdBufferElemCount(n, cmdIdx)*/(int)cmd.getElemCount();
//                final int idxOffset = /*drawData.getCmdListCmdBufferIdxOffset(n, cmdIdx)*/(int)cmd.getIdxOffset();
//                final int vtxOffset = /*drawData.getCmdListCmdBufferVtxOffset(n, cmdIdx)*/(int)cmd.getVtxOffset();
//                final long indices = /*idxOffset * (long) ImDrawData.sizeOfImDrawIdx()*/cmd.getIdxOffset()* ImGuiUtil.ImDrawIDX__;
//                final int type = ImGuiUtil.ImDrawIDX__ == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT;
//
...
////        var ddata = fontAtlas.getTexData();
////        final ByteBuffer pixels = /*fontAtlas.getTexDataAsRGBA32(width, height)*/MemorySegment.ofAddress(ImGuiUtil.getSWIGTYPE_p_void(ddata.GetPixels()))
////                .reinterpret(ddata.GetSizeInBytes())
////                .asByteBuffer();
////        int width = ddata.getWidth();
////        int height = ddata.getHeight();
////
...
//////        var ref = new ImTextureRef();
//////        ref.set_TexID(BigInteger.valueOf(data.fontTexture));
////        fontAtlas.getTexID().set_TexID(BigInteger.valueOf(data.fontTexture));
////
...
////            io.getFonts().getTexID().set_TexID(BigInteger.valueOf(0));
////            data.fontTexture = 0;
////        }
////    }
//
...
//    {
//        int gl_tex_id = tex.getTexID().intValue();
//        glDeleteTextures(gl_tex_id);
//
//        // Clear identifiers and mark as destroyed (in order to allow e.g. calling InvalidateDeviceObjects while running)
//        tex.SetTexID(BigInteger.valueOf(-1)); //ImTextureID_Invalid
//        tex.SetStatus(ImTextureStatus.ImTextureStatus_Destroyed); //ImTextureStatus_Destroyed
//    }
//
//    static void ImGui_ImplOpenGL3_UpdateTexture(ImTextureData tex)
//        {
//            // FIXME: Consider backing up and restoring
//            var stat = tex.getStatus();
//            if (stat == ImTextureStatus.ImTextureStatus_WantCreate || stat== ImTextureStatus.ImTextureStatus_WantUpdates)
//            {
////    #ifdef GL_UNPACK_ROW_LENGTH // Not on WebGL/ES //This Port is only for modern desktops
...
//            }
//
//            if (stat == ImTextureStatus.ImTextureStatus_WantCreate)
//            {
//                // Create and upload new texture to graphics system
//                //IMGUI_DEBUG_LOG("UpdateTexture #%03d: WantCreate %dx%d\n", tex->UniqueID, tex->Width, tex->Height);
//                var id = tex.GetTexID().intValue();
//                var format = tex.getFormat();
////                IM_ASSERT(tex->TexID == ImTextureID_Invalid && tex->BackendUserData == nullptr);
//                assert id == -1 && tex.getBackendUserData() == null;
////                IM_ASSERT(tex->Format == ImTextureFormat_RGBA32);
//                assert format == ImTextureFormat.ImTextureFormat_RGBA32;
//                var pixels = tex.GetPixels();
//                int gl_texture_id = 0;
//
//                // Upload texture to graphics system
//                // (Bilinear sampling is required by default. Set 'io.Fonts->Flags |= ImFontAtlasFlags_NoBakedLines' or 'style.AntiAliasedLinesUseTex = false' to allow point/nearest sampling)
//                var last_texture = glGetInteger(GL_TEXTURE_BINDING_2D);
//                gl_texture_id = glGenTextures();
//                glBindTexture(GL_TEXTURE_2D, gl_texture_id);
//                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
//                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
//                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
//                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
//                nglTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, tex.getWidth(), tex.getHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE, ImGuiUtil.getSWIGTYPE_p_void(pixels));
//
//                // Store identifiers
//                tex.SetTexID(BigInteger.valueOf(gl_texture_id));
//                tex.SetStatus(ImTextureStatus.ImTextureStatus_OK);
//
//                // Restore state
//                glBindTexture(GL_TEXTURE_2D, last_texture);
//            }
//            else if (stat == ImTextureStatus.ImTextureStatus_WantUpdates)
//            {
...
////    #if GL_UNPACK_ROW_LENGTH // Not on WebGL/ES //This port is only for modern desktops
//                glPixelStorei(GL_UNPACK_ROW_LENGTH, tex.getWidth());
//                for (ImTextureRect r : new ImGuiUtil.ImTextureRectVector(tex.getUpdates())){
//                    glTexSubImage2D(GL_TEXTURE_2D, 0, r.getX(), r.getY(), r.getW(), r.getH(), GL_RGBA, GL_UNSIGNED_BYTE, tex -> GetPixelsAt(r.x, r.y));
//                }
//                glPixelStorei(GL_UNPACK_ROW_LENGTH, 0));
////    #else
...
//    //--------------------------------------------------------------------------------------------------------
//
//    private final class RendererRenderWindowFunction implements ImGuiUtil.SWIGTYPE_p_f_p_ImGuiViewport_p_void__voidImpl.call {
////        @Override
...
//            if ((vp.getFlags() & ImGuiViewportFlags_.ImGuiViewportFlags_NoRendererClear) == 0) {
...
//        imgui.GetPlatformIO().setRenderer_RenderWindow(new ImGuiUtil.SWIGTYPE_p_f_p_ImGuiViewport_p_void__voidImpl(new RendererRenderWindowFunction(), Arena.global()));
//    }
//
